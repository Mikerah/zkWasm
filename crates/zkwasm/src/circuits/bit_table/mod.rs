...
impl<F: FieldExt> BitTableConfig<F> {
    ...
    pub(crate) fn configure(meta: &mut ConstraintSystem<F>, rtable: &RangeTableConfig<F>) -> Self {
        ...
        meta.create_gate("bit table: 2. acc u32", |meta| {
            ...
            macro_rules! compose_u32 {
                ($col:expr) => {
                    (0..4)
                        ...
                        .fold(None, |acc, x| {
                            acc.map_or(Some(x.clone()), |acc| Some(acc + x))
                        })
                        .unwrap()
                };
            }
            ...
            macro_rules! compose_u32_if_binary {
                ($col:ident) => {
                    fixed_curr!(meta, u32_sel)
                        //* (constant_from!(1) - curr!(meta, is_popcnt))
                        * is_bit.clone()
                        * (compose_u32!($col) - curr!(meta, $col))
                };
            }
            ...
            macro_rules! acc_u32_if_popcnt {
                ($col:ident) => {
                    fixed_curr!(meta, u32_sel)
                        // * (curr!(meta, is_popcnt))
                        * is_popcnt
                        * (acc_u32!($col) - curr!(meta, $col))
                };
            }

            vec![
                compose_u32_if_binary!(left),
                compose_u32_if_binary!(right),
                compose_u32_if_binary!(result),
        ...
        meta.create_gate("bit table: 3. acc u64", |meta| {
            ...
            macro_rules! compose_u64_if_binary {
                ($col: expr) => {
            ...
            vec![
                compose_u64_if_binary!(left),
                compose_u64_if_binary!(right),
                compose_u64_if_binary!(result),
